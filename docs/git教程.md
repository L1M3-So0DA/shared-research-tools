...
*注意：已经被git跟踪的文件，即使在`.gitignore`中添加，也不会被忽略。*

如果某个文件已经被 Git 跟踪，需要先执行 `git rm --cached <file>` 取消跟踪，再提交一次，`.gitignore` 才会对它生效。

### 基本概念

- **工作区（Working Directory）**：当前正在编辑的文件所在的目录。
- **暂存区（Staging Area）**：一个临时区域，用于保存即将提交的更改。
- **本地仓库（Local Repository）**：存储提交历史的地方，包含所有提交记录和版本信息。
- **远程仓库（Remote Repository）**：托管在服务器上的仓库，可以与本地仓库进行同步。
- **HEAD**：指向当前分支的最新提交，代表当前工作区的状态。
- **分支（Branch）**：Git 中的分支是一个指向提交的指针，允许你在同一个项目中同时进行多个开发任务，而不会互相干扰。

### 查看状态和差异

在开始提交之前，先看当前仓库的状态和差异，通常更容易判断自己改了什么：

```bash
git status           # 查看工作区、暂存区和未跟踪文件的状态
...
如果需要查看之前的提交历史，可以直接点击左侧活动栏的"源代码管理"图标，安装 **GitLens** 后，可以可视化查看提交历史，并且可以方便地检出到指定版本。

如果需要查看之前的版本，可以右键点击指定的提交，选择"签出"，也可以使用以下命令：

```bash
git log --oneline           # 查看提交历史，找到需要检出的版本号
git checkout <commit_id>    # 检出到指定提交
```

检出到历史提交后，HEAD 会进入 detached HEAD 状态。这个状态适合临时查看历史代码；如果要基于这个版本继续开发，建议先创建一个新分支：

```bash
git switch -c <branch_name>    # 基于当前检出位置创建新分支
```

如果只是临时查看历史，完成后可以切回主分支或原来的开发分支：

```bash
git switch main            # 切换回主分支
```

如果你的主分支名称不是 main，请把这里替换成实际的分支名。

*注意：在检出之前，请确保已经保存了当前工作区的更改，以免丢失未提交的修改。检出到历史提交后，如果需要继续开发，最好先创建新分支再修改代码。*

### 撤销提交

如果最近的提交还没有推送到远程仓库，可以用 reset 回退本地历史：
...
如果提交已经推送到远程仓库，通常更适合使用 revert 生成一个反向提交，而不是直接改写历史：

```bash
git revert <commit_id>    # 撤销指定提交，并生成一个新的反向提交
git revert HEAD           # 撤销最近一次提交
```

*注意：使用 `--hard` 选项会丢失未提交的更改，请谨慎使用。已经推送到远程的提交，尽量不要直接 reset 改写历史。*

### 远程操作

在推送和拉取之前，可以先查看远程仓库地址和本地分支的跟踪关系：

```bash
git remote -v        # 查看远程仓库地址
git fetch origin     # 拉取远程更新，但不自动合并
git branch -vv       # 查看本地分支与远程跟踪分支的关系
```
...
git push -u origin <branch_name>    # 第一次推送分支并建立跟踪关系
```
...
如果分支已经建立了跟踪关系，也可以直接使用 `git pull`。*注：在VSCode中有“拉取”和“从所有远程储存库中拉取”选项，一般情况下选择“拉取”，后续章节会介绍“从所有远程储存库中拉取”。*

*注：如果你的默认分支不是 main，请把示例中的 main 替换成实际分支名。*
...
git switch feature-xyz    # 切换到名为feature-xyz的分支
git switch -c <branchname>    # 创建并切换到新分支
git switch -c feature-xyz    # 创建并切换到名为feature-xyz的新分支
```
...
如果合并时出现冲突，VSCode 会在冲突文件中标出冲突位置。可以根据提示选择保留当前更改、传入更改，或者同时保留两边内容。处理完成后，使用 `git add` 标记冲突已解决，再提交一次合并结果。
...
*注意：在删除分支时，请确保已经合并了该分支的更改，以免丢失未合并的修改。如果该分支已经推送到远程，也要确认远程分支不再需要。*
...
git restore 命令主要用于恢复文件内容，可以把工作区或暂存区的修改恢复到上一次提交的状态，适合撤销单个文件或部分文件的更改。

### 恢复工作区文件

如果文件已经修改，但还没有加入暂存区，可以直接恢复到最近一次提交：

```bash
git restore <file>    # 恢复工作区中的指定文件
git restore .         # 恢复当前目录下所有未暂存的修改
```

### 取消暂存

如果文件已经执行 `git add`，但还想保留工作区修改，可以取消暂存：
...
如果要从某个提交中恢复文件内容，可以指定 `source`：

```bash
git restore --source=<commit_id> <file>    # 从指定提交恢复文件到工作区
git restore --source=HEAD~1 --staged --worktree <file>    # 同时恢复暂存区和工作区
```

*注意：git restore 主要针对文件，不用于切换分支。它可以替代一部分 `git checkout <file>` 的文件恢复场景，但如果你要切换分支，仍然应该使用 `git switch`。执行恢复前，最好先用 `git status` 确认文件状态，避免误删修改。*
...
```bash
git worktree add -b <branch_name> <path> <start_point>    # 创建新分支并检出到新的目录
git worktree add -b feature-new ../feature-new main    # 基于本地main创建feature-new分支并检出到../feature-new目录
git fetch origin    # 先更新远程信息，再基于 origin/main 创建工作树
git worktree add -b hotfix-new ../hotfix-new origin/main    # 基于远程主分支创建新的工作树
```

如果你的本地主分支已经和远程同步，也可以直接把 `main` 作为起点；如果想基于远程最新提交创建工作树，先执行 `git fetch origin`，再使用 `origin/main` 作为起点。
